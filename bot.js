...
/* 			var descriptionAbout = "```Custom Open Source Discord Bot built with Discord.js for our custom Minecraft community and server.\n\nFor more information including current and planned features visit the link above.\n\nBig Brother is Watching You!```";
			msg.channel.send({embed: {
			    ...
			    author: {
					name: bot_nickname+" - About",
					icon_url: bot_logo_square,
					url: "https://www.worldautomation.net"
			    },
			    //title: bot_nickname,
			    //url: info_website,
			    description: descriptionAbout,
...
/* 			var descriptionAbout = "```Custom Open Source Discord Bot built with Discord.js for our custom Minecraft community and server.\n\nFor more information including current and planned features visit the link above.\n\nBig Brother is Watching You!```";
		...
		var exec = require('child_process').exec;
<<<<<<< HEAD
		var replaceThisColorGreen = "�		function puts(error, stdout, stderr) { 
...
=======
		function puts(error, stdout, stderr) { 
			msg.channel.send("<:main_computer:420575980198035456> <:terminalreal:421547027051184128>  `[Main Computer] Bot @ WA.Net#` "+mentionCommandAuthor+" `Pinging "+host+" with a few packets... please wait...`\n```ldif\n"+stdout+"```"); 
>>>>>>> cce016ff1a052c54ced49e9efb4d7eec18aedc6c
		}
		if (systemOS === "win32") {
			exec("ping -n 5 "+host, puts);
		} else {
...
/* 			embed: {
		...
		var exec = require('child_process').exec;
<<<<<<< HEAD
		var replaceThisColorGreen = "�		var replaceThisColorYellow = "�		function puts(error, stdout, stderr) { 
...
>>>>>>> cce016ff1a052c54ced49e9efb4d7eec18aedc6c
		...
		var exec = require('child_process').exec;
<<<<<<< HEAD
		var replaceThisColorGreen = "�		var replaceThisLineBreak = "		function puts(error, stdout, stderr) { 
...
/* 			  for (var i = 0; i < results.length; i++) {
	...
	switch(cmd) {
		case "skip":
			...
			break;
		case "remove":
			msg.channel.send("<:main_computer:420575980198035456> :headphones: :wastebasket:  `[Main Computer] Radio @ WA.Net# Not yet implemented!`");
			break;
		case "nowplaying":
			radioNowPlaying("422898611106480139");
			break;
	...
	youTube.search(searchTerm, 1, function(error, result) {
		if (error) {
			...
			result['items'].forEach(function (video) {
				...
				video.snippet.title = video.snippet.title.replace(/[^a-zA-Z0-9-_]/g, '_').replace("_-_", "-").replace("__-__","-");
				var videoDownload = video.snippet.title;
				var playerQueryIntro = "<:main_computer:420575980198035456> <:ytdl:423017037393166336>  `[Main Computer] YouTube Download @ WA.Net# Starting download for "+videoNamePretty+"...`";
				...
				ytdl.getInfo(videoUrl, function(err, info){
					...
					function completeMessage() {
						...
						request.post('https://www.googleapis.com/urlshortener/v1/url?key='+api_google_shortener, {
						  json: {
							'longUrl': 'https://radio.worldautomation.net/youtube/'+video.snippet.title+'.mp4'
						  }
						}, function (error, response, body) {
						  if(error) {
							console.log(error)
						  } else {
							msg.channel.send("<:main_computer:420575980198035456> <:ytdl:423017037393166336> :white_check_mark:  `[Main Computer] YouTube Download @ WA.Net# Added download request from ` "+mentionCommandAuthor+" ` to the server...` ```"+videoNamePretty+"\nDownloaded and encoded into MP4 (Video)...\nAdded to WA.Net Downloads...\nEnjoy!```Watch it in your Browser or Download it Here -> "+body.id);	
							//console.log(response.statusCode, body)
						  }
						})
						//
